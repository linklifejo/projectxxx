    ...
    public void onBindViewHolder(MultiImageAdapter.ViewHolder holder, int position) {
        ...
        holder.image.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
               ...
               // imgFile = (File) holder.image.setImageResource(image_uri.toString());
                imgFilePath = image_uri.getPath();
               imgFile = (FGlide.with(view).asBitmap();
                Integer i =Integer.parseInt(image_uri.getPath().substring(image_uri.getPath().lastIndexOf('/')+1));

                aa();
            //    int i =(int)image_uri.getPath().substring(image_uri.getPath().lastIndexOf('/')+1);
                Toast.makeText(mContext," " + image_uri.getPath().substring(image_uri.getPath().lastIndexOf('/')+1), Toast.LENGTH_SHORT).show();
            }
        });
    }
